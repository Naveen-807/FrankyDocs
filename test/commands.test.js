...
describe("parseCommand", () => {
    ...
    it("parses limit buy", () => {
        ...
        if (r.ok)
            expect(r.value).toMatchObject({ type: "LIMIT_BUY", qty: 50, price: 1.02 });
    });
    ...
    it("parses payout split", () => {
        const r = parseCommand("DW PAYOUT_SPLIT 10 USDC TO 0x0000000000000000000000000000000000000001:50,0x0000000000000000000000000000000000000002:50");
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(r.value.type).toBe("PAYOUT_SPLIT");
    });
    ...
    it("parses signer add", () => {
        ...
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(r.value).toMatchObject({ type: "SIGNER_ADD", weight: 2 });
    });
    it("parses quorum", () => {
